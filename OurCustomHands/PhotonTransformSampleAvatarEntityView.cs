...
namespace Photon.Pun
{
    ...
    public class PhotonTransformSampleAvatarEntityView : MonoBehaviourPun, IPunObservable
    {
        ...
        public SampleAvatarEntity m_SampleAvatarEntity;
        private List<Transform> m_Lods = new List<Transform>();
        public bool m_RenderAndColliderObjectActive = true;
        private AvatarLOD m_AvatarLOD;

        private Transform m_SkeletonLeft;
        private Transform m_SkeletonRight;
        [Tooltip("Indicates if localPosition and localRotation should be used. Scale ignores this setting, and always uses localScale to avoid issues with lossyScale.")]
        public bool m_UseLocal;

        bool m_firstTake = false;
        private bool m_AllCollected = false;
        ...
        public void Awake()
        {
            ...
            m_AvatarHead = this.GetComponent<PhotonTransformView>();
            CollectData();
        }

        private void CollectData()
        {
            if (m_AllCollected == false)
            {
                if (m_AvatarLOD == null)
                {
                    m_AvatarLOD = m_SampleAvatarEntity.GetComponentInChildren<AvatarLOD>();
                }
                ...
                if (m_SkeletonLeft == null)
                {
                    m_SkeletonLeft = this.transform.SearchComponent<Transform>("OurAvatarPlayerHandSkeletonLeft");
                }
                if (m_SkeletonRight == null)
                {
                    m_SkeletonRight = this.transform.SearchComponent<Transform>("OurAvatarPlayerHandSkeletonRight");
                }

                m_AllCollected = (m_AvatarLOD != null && m_SkeletonLeft != null && m_SkeletonRight != null && m_Lods.Count > 0);
            }
        }
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            ...
            if (stream.IsWriting)
            {
                stream.SendNext(this.m_RenderAndColliderObjectActive);
            ...
            {
                RenderAndColliderForShadow((bool)stream.ReceiveNext());
        ...
        private void RenderAndColliderForShadow(bool render)
        {
            CollectData();

            if (m_AllCollected == true)
            {
                m_SkeletonLeft.SetActive(render);
                m_SkeletonRight.SetActive(render);
                if (render == false)
                {
                    m_Lods.ForEach(e => e.gameObject.SetActive(false));
                }
                else
                {
                    m_AvatarLOD.overrideLOD = true;
                    // force it on
                    var lodRendered = m_Lods.Find(e => e.gameObject.activeInHierarchy == true);
                    if (lodRendered == null)
                    {
                        m_Lods[0].SetActive(true);
                    }
                }

                if (this.m_RenderAndColliderObjectActive != render)
                {
                    Debug.LogError($"This is teleport {this.m_RenderAndColliderObjectActive}", this);
                    this.m_RenderAndColliderObjectActive = render;
                    m_firstTake = true;
                }
            }
        }
