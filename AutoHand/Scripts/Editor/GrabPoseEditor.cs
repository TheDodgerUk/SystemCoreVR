...
namespace Autohand
{
    ...
    public class GrabPoseEditor : Editor
    {
        ...
        public void ShowHandEditorHand()
        {
            ...
            if (GUILayout.Button("Apply Pose Left"))
            {
                ApplyPose(true);
            }

            if (GUILayout.Button("Apply Pose Right"))
            {
                ApplyPose(false);
            }
        }

        private void ApplyPose(bool left)
        {
            var hands = FindObjectsOfType<Autohand.Hand>().ToList();
            foreach (var itemHand in hands)
            {
                List<Transform> trans = new List<Transform>();
                foreach (var finger in itemHand.fingers)
                {
                    foreach (var joint in finger.fingerJoints)
                    {
                        trans.Add(joint.transform);
                    }
                }
                if (left == true)
                {
                    for (int i = 0; i < grabbablePose.poseScriptable.leftPose.posePositions.Length; i++)
                    {
                        trans[i].localRotation = grabbablePose.poseScriptable.leftPose.poseRotations[i];
                        trans[i].localPosition = grabbablePose.poseScriptable.leftPose.posePositions[i];
                    }
                }
                else
                {
                    for (int i = 0; i < grabbablePose.poseScriptable.rightPose.posePositions.Length; i++)
                    {
                        trans[i].localRotation = grabbablePose.poseScriptable.rightPose.poseRotations[i];
                        trans[i].localPosition = grabbablePose.poseScriptable.rightPose.posePositions[i];
                    }
                }
            }
        }
