...
namespace Autohand
{
    ...
    public struct HandPoseData
    {
        ...
        public static readonly List<string> VALID_FINGERS = new List<string>()
    {
            { "Thumb" },
            { "Thumb1" },
            { "Thumb2" },
        ...
        public static bool IsValidFinger(Transform trans)
        {
            foreach (var fingerEnd in VALID_FINGERS)
            {
                if (trans.name.EndsWith(fingerEnd) == true)
                {
                    return true;
                }
            }
        ...
        public void SetPose(Hand hand, Transform relativeTo = null)
        {
            ...
            void AssignChildrenPose(Transform obj, HandPoseData pose)
            {
                ...
                {
                    i++;
                    if (i < pose.posePositions.Length)
                    {
                    ...
                    {
                        Debug.LogError($"index outof range posePositions:{pose.posePositions.Length}, i {i}");
                    }
                    for (int j = 0; j < obj.childCount; j++)
                    {
